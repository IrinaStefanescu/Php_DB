...
if(isset($_POST["id"])){
    $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
        ...
        $nume = $_POST["nume_departament"];
        $email = $_POST["email_departament"];


        $stmt =  $connection->prepare("update  departament set nume_departament= '$nume',
                                        email_departament = '$email' 
                                         where  id_departament = '$id'"); 
        $stmt->execute();
        
}

$query = "select id_departament, departament.nume_departament, departament.email_departament from departament where id_departament = '".$id."'";
 ...
 $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
    ...
    header("Location:http://localhost/BDIrina/departament.php");
}
...
<body>
    ...
    <div class="stage" style="overflow-y:auto;">
        ...
        <h2 style="text-align:center;width:100%;"> DEPARTAMENT </h2>
            <form action="editDepartament.php" method="post">
           <input type="hidden" name="id" value="<?php echo $results[0];?>">
           
            <div class="row">
                <label> Nume </label>
                <input type="text" name="nume_departament" value="<?php echo $results[1];?>">
            </div>
           
            <div class="row">
                <label> E-mail </label>
                <input type="text" name="email_departament" value="<?php echo $results[2];?>" >
            </div>
