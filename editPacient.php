...
if(isset($_POST["id"])){
    $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
        ...
        $telefon = $_POST["telefon_pacient"];
        $varsta = $_POST["varsta_pacient"];


        $stmt =  $connection->prepare("update  pacient set nume_pacient= '$nume',
                                        prenume_pacient = '$prenume',
                                        telefon_pacient =  '$telefon',
                                        varsta = '$varsta'  where  id_pacient = '$id'"); 
        $stmt->execute();
        
}

$query = "select * from pacient where id_pacient = '".$id."'";
 ...
 $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
    ...
    header("Location:http://localhost/BDIrina/pacient.php");
}
...
<body>
    ...
    <div class="stage" style="overflow-y:auto;">
        <div class="content">
        <h2 style="text-align:center;width:100%;"> DEPARTAMENT </h2>
            <form action="editPacient.php" method="post">
           <input type="hidden" name="id" value="<?php echo $results[0];?>">
           
            <div class="row">
                <label> Nume </label>
                <input type="text" name="nume_pacient" value="<?php echo $results[1];?>">
            </div>
           
            <div class="row">
                <label> Prenume </label>
                <input type="text" name="prenume_pacient" value="<?php echo $results[2];?>" >
            </div>

            <div class="row">
                <label> Varsta </label>
                <input type="text" name="varsta_pacient" value="<?php echo $results[3];?>" >
            </div>

            <div class="row">
                <label> Telefon </label>
                <input type="text" name="telefon_pacient" value="<?php echo $results[4];?>" >
            </div>
