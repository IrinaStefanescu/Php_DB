...
if(isset($_POST["id"])){
    $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
        ...
        $etaj = $_POST["etaj"];
        $nrpacienti = $_POST["nr_pacienti"];


        $stmt =  $connection->prepare("update  saloane set etaj= '$etaj',
                                        nr_pacienti = '$nrpacienti' 
                                         where  id_salon = '$id'"); 
        $stmt->execute();
        
}

$query = "select * from saloane where id_salon = '".$id."'";
 ...
 $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
    ...
    header("Location:http://localhost/BDIrina/saloane.php");
}
...
<body>
    ...
    <div class="stage" style="overflow-y:auto;">
        <div class="content">
        <h2 style="text-align:center;width:100%;"> SALON </h2>
            <form action="EditSalon.php" method="post">
           <input type="hidden" name="id" value="<?php echo $results[0];?>">
           
            <div class="row">
                <label> Etaj </label>
                <input type="text" name="etaj" value="<?php echo $results[1];?>">
            </div>
           
            <div class="row">
                <label> Nr pacienti </label>
                <input type="text" name="nr_pacienti" value="<?php echo $results[2];?>" >
            </div>
