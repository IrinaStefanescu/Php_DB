...
$query = "select pacient.id_pacient, pacient.nume_pacient, pacient.prenume_pacient, pacient.varsta, pacient.telefon_pacient, tratament.nume_tratament from pacient join tratament using(cod_tratament)";
if(isset($_GET["sort"])){
    if($_GET["sort"] == 1)
    $query = "select pacient.id_pacient, pacient.nume_pacient, pacient.prenume_pacient, pacient.varsta, pacient.telefon_pacient, tratament.nume_tratament from pacient join tratament using(cod_tratament) order by pacient.nume_pacient asc";
    elseif ($_GET["sort"] == 2) {
        $query = "select  pacient.id_pacient, pacient.nume_pacient, pacient.prenume_pacient, pacient.varsta, pacient.telefon_pacient, tratament.nume_tratament from pacient join tratament using(cod_tratament) order by pacient.prenume_pacient asc";
...
<body>
    ...
    <div class="content">
    <h2 style="text-align:center;width:100%;"> PACIENT </h2>

            <div class="select-wrapper">
                <select>
                   
                         <option value="0">Fara sortare</option>
                         <option <?php if(isset($_GET["sort"]) && $_GET["sort"] == 1) echo 'selected="selected"'; ?> value="1">Sortare dupa nume A-Z</option>
                         <option  <?php  if(isset($_GET["sort"]) && $_GET["sort"] == 2) echo 'selected="selected"'; ?> value="2">Sortare dupa prenume A-Z</option>
                </select>
            <table>
             ...
             <?php foreach($results as $linie){ ?>
                    <tr>
                        ...
                        <td>
                        <a href="/BDIrina/editPacient.php?id=<?php echo $linie[0];?>">Editeaza</a>
                        </td>

                        <td>
                        <a href="/BDIrina/stergereIntrare.php?tabel=pacient&id_pacient=<?php echo $linie[0];?>">Sterge</a>
                        </td>
                    </tr>
            <?php } ?>    
...
$('select').on('change',function(ev){
    window.location.href="pacient.php?sort="+this.value;
})
