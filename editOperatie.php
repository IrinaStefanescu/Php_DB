...
if(isset($_POST["id"])){
    $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
        ...
        $stmt =  $connection->prepare("update  operatii set durata = '$nd',
                                        risc1 = '$r1',
                                        risc2 = '$r2',
                                        risc3 = '$r3' 
                                        
                                         where   id_operatie = '".$id."'"); 
        $stmt->execute();
        
}
$query = "select * from operatii where id_operatie = '".$id."'";
 ...
 $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
    ...
    header("Location:http://localhost/BDIrina/operatii.php");
}
...
<body>
    ...
    <div class="stage" style="overflow-y:auto;">
        ...
        <h2 style="text-align:center;width:100%;"> DEPARTAMENT </h2>
            <form action="editOperatie.php" method="post">
           <input type="hidden" name="id" value="<?php echo $results[0];?>">
           
            <div class="row">
                <label> Durata </label>
                <input type="text" name="durata" value="<?php echo $results[1];?>">
            </div>
           
            <div class="row">
                <label> Risc1 </label>
                <input type="text" name="risc1" value="<?php echo $results[2];?>" >
            </div>

            <div class="row">
                <label> Risc2 </label>
                <input type="text" name="risc2" value="<?php echo $results[3];?>" >
            </div>

            <div class="row">
                <label> Risc3 </label>
                <input type="text" name="risc3" value="<?php echo $results[4];?>" >
            </div>
...
$('select').on('change',function(ev){
    window.location.href="operatii.php?sort="+this.value;
})
