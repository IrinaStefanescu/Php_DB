...
if(isset($_POST["id"])){
    $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
        $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $nume = $_POST["nume_doctor"];
        $prenume = $_POST["prenume_doctor"];
        $telefon = $_POST["telefon_doctor"];


        $stmt =  $connection->prepare("update  doctori set nume_doctor= '$nume',
                                        prenume_doctor = '$prenume',
                                        telefon_doctor =  '$telefon' where  id_doctor = '$id'"); 
        $stmt->execute();
        
}


$query = "select doctori.id_doctor,doctori.nume_doctor,doctori.prenume_doctor,doctori.telefon_doctor from doctori where id_doctor = '".$id."'";
 ...
 $connection = new PDO("mysql:host=localhost".";dbname=spital","root", "");
    ...
    header("Location:http://localhost/BDIrina/doctori.php");
}
...
<body>
    ...
    <div class="stage" style="overflow-y:auto;">
        <div class="content">
        <h2 style="text-align:center;width:100%;"> DOCTOR </h2>
            <form action="editDoctor.php" method="post">
           <input type="hidden" name="id" value="<?php echo $results[0];?>">
           
            <div class="row">
                <label> Nume </label>
                <input type="text" name="nume_doctor" value="<?php echo $results[1];?>">
            </div>
           
            <div class="row">
                <label> Prenume </label>
                <input type="text" name="prenume_doctor" value="<?php echo $results[2];?>" >
            </div>
            <div class="row">
                <label> Telefon </label>
                <input type="text" name="telefon_doctor" value="<?php echo $results[3];?>">
            </div>
